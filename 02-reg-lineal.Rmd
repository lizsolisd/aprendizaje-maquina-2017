...
```{r}
#función cerradura
#primero debes llamar la fn con el param de más afuera y después el de adentro
#ejemplo fn potencia
...
```{r,  fig.width=4,fig.asp=0.7}
#minimiza la funeción rss y el algoritmo inicia en (0,0) (semilla)
#con el método BFGS, puede ser el que quieras
#Ojo: esto solo puede ser como fue construida la fn rss (que sólo le
#tengas que dar como input las betas)
...
$$f_{\beta} (x) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \cdots + \beta_p x_p$$
**Nota:** la derivada es con respecto a $\beta$
...
$$s_j^2 = \frac{1}{N-1}\sum_{i=1}^N (x_j^{(i)}- \bar{x}_j )^2$$
...
pr_entrena_s <- 
  ...
  #para pasar las variables  a renglones (columnas a renglones dependiendo de la variable)
...
- * Conviene hacer todo el análisis con los datos estandarizados xq asi es transparente 
el moviento si es un desviación (y no tengo que hacer la conversión como decir si sube 1.2 unis el lcavol 
en lugar de decir una desviación)
- * Podemos decir que lpsa no contribuye tanto y lcavol si xq es el triple de la media (ver lo  q sale en el summary)
...
- Si tomo pocos vecinos más cercanos hay mucha variablidad
- Si tomo muchos ya existe la parte del sesgo, pues ya están muy distantes las observaciones y ya no tienen q ver entre ellos
...
se distribuyen de manera rara en el espacio de entradas.
```

- Entre más diemsiones tienes, los datos suelen irsea la frontera y el centro está prácticamente vacío
...
set.seed(111)
sims_1 <- lapply(1:40, function(i) runif(1000, -1, 1) )
...
es mucho mejor que la de 1 vecino más cercano. Esto es porque el modelo **explota la estructura aproximadamente lineal del problema.**
